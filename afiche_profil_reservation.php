...
$query = $requete->prepare('SELECT id, prenom, nom, created_at FROM users WHERE id = :id');
...
$prenom = $user['prenom'];
$nom = $user['nom'];
$photo = !empty($user['photo']) ? $user['photo'] : 'images/default-profile.jpg';
...
<head>
    ...
    <style>
        ...
        .back-btn { position: absolute; left: 20px; top: 20px; color: white; font-size: 24px; }
        .profile-title { font-weight: 700; text-shadow: 2px 2px 4px rgba(0,0,0,0.5); }
        .profile-container { max-width: 1200px; margin: 0 auto; padding: 0 20px; }
        .profile-card { display: flex; align-items: center; background: white; border-radius: 10px; padding: 20px; margin-bottom: 30px; box-shadow: 0 3px 10px rgba(0,0,0,0.1); }
        .profile-img { width: 80px; height: 80px; border-radius: 50%; object-fit: cover; margin-right: 20px; border: 3px solid #2ecc71; }
        .reservation-card { background: white; border-radius: 10px; padding: 25px; margin-bottom: 20px; box-shadow: 0 3px 10px rgba(0,0,0,0.1); position: relative; transition: all 0.3s ease; }
        ...
        .reservation-title { font-weight: 600; color: #2c3e50; font-size: 18px; }
        .reservation-date { color: #7f8c8d; font-size: 14px; }
        .reservation-detail { margin: 10px 0; display: flex; }
        .detail-label { font-weight: 600; color: #2c3e50; min-width: 180px; }
        .reservation-actions { display: flex; justify-content: flex-end; margin-top: 15px; }
        .action-btn { border: none; background: none; cursor: pointer; font-size: 16px; margin-left: 15px; transition: all 0.2s; }
        .action-btn:hover { transform: scale(1.1); }
        .edit-btn { color: #f39c12; }
        .delete-btn { color: #e74c3c; }
        .no-reservations { text-align: center; padding: 50px 0; }
        .no-reservations i { font-size: 50px; color: #bdc3c7; margin-bottom: 20px; }
        .transport-icon { color: #2ecc71; margin-right: 8px; width: 20px; text-align: center; }
        @media (max-width: 768px) {
            .profile-header { height: 200px; }
            .profile-card { flex-direction: column; text-align: center; }
            .profile-img { margin-right: 0; margin-bottom: 15px; }
            .reservation-detail { flex-direction: column; }
...
<div class="profile-container">
    <div class="profile-card">
        ...
        <div class="profile-info">
            ...
            <p class="text-muted">Membre depuis <?php echo date('d/m/Y', strtotime($created_at)); ?></p>
        </div>
    </div>
                ...
                <div class="col">
                    <div class="reservation-card">
                        ...
                        <div class="reservation-actions">
                            ...
                            <a href="afiche_profil_reservation.php?del=<?php echo $reservation['id']; ?>&id=<?php echo $users_id; ?>" class="delete-btn action-btn" title="Supprimer" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette réservation ?')"><i class="fas fa-trash-alt"></i></a>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
    ...
    <?php else: ?>
        <div class="no-reservations">
            ...
            <a href="reservation.php" class="btn btn-primary mt-3"><i class="fas fa-plus-circle"></i> Nouvelle réservation</a>
        </div>
    <?php endif; ?>
</div>
